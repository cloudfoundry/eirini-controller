...
var _ = Describe("PodCrashEvents [needs-logs-for: eirini-api, eirini-controller]", func() {
	...
	When("a crashing app is deployed", func() {
		...
		It("populates the events with relevant information", func() {
			...
			Expect(crash.Labels).To(HaveKeyWithValue("workloads.cloudfoundry.org/instance-index", "0"))
			Expect(crash.Annotations).To(HaveKeyWithValue("workloads.cloudfoundry.org/process-guid", fmt.Sprintf("%s-%s", lrpGUID, lrpVersion)))
