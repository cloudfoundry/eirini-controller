...
/bin/bash "${CODEGEN_PKG}/generate-groups.sh" all \
  code.cloudfoundry.org/eirini-controller/pkg/generated code.cloudfoundry.org/eirini-controller/pkg/apis \
  eirini:v1 \
  --output-base "$(dirname "${BASH_SOURCE[0]}")/.." \
  --go-header-file "${EIRINI_CONTROLLER_ROOT}/hack/boilerplate.go.txt"

cp -R $EIRINI_CONTROLLER_ROOT/code.cloudfoundry.org/eirini-controller/pkg/generated $EIRINI_CONTROLLER_ROOT/pkg
cp -R $EIRINI_CONTROLLER_ROOT/code.cloudfoundry.org/eirini-controller/pkg/apis/* $EIRINI_CONTROLLER_ROOT/pkg/apis/
